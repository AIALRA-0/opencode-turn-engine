...
| 2 | Codex exec-server 研究/迁移 | bash 和文件内容读写已接源码构建 sidecar | 已查明“握手失败”不是泛泛失败：全局安装的 `codex-cli 0.125.0-alpha.3` 打印 ws 地址但不完成当前客户端需要的 HTTP 101；本机源码构建的 Codex binary 可以 `/readyz`、`initialize`、`process/start/read` 和 FS API。AIALRA 主服务默认连接 `aialra-codex-exec-server.service` 的 `ws://127.0.0.1:12650`；bash、read/write/edit/apply_patch 优先走 Codex exec-server，失败再 fallback。目录列举还未接 `fs/readDirectory`。 | `packages/opencode/src/tool/codex-exec-server.ts`、`packages/opencode/src/tool/codex-fs.ts`、`aialra-codex-exec-server.service`、`test/tool/codex-exec-server.test.ts` |
...
| 4 | debug1 原版 OpenCode A/B | 已部署并已跑三方报告 | `debug1.aialra.online` 已部署原版 anomalyco/opencode，对照组和 AIALRA fork 端口、systemd、数据目录、环境变量、日志全部隔离。三方 A/B 脚本已能跑 Codex CLI、debug1 原版 OpenCode、AIALRA fork。最新 7 prompt 报告显示：Codex CLI 和 AIALRA fork 都是 7/7 成功、0 次等待审批、0 次越界写入、7/7 turn 终态；原版 OpenCode 仍在越界写入、混合任务、情绪化越界任务中停在审批等待。 | `aialra/turn-observability/scripts/run-ab-comparison.mjs`、`ab-reports/ab-comparison-20260519153559.md`、`aialra-opencode-debug1-*` systemd services |
...
| exec-server adapter | 新增 Codex exec-server JSON-RPC client，支持 `initialize`、`initialized`、`process/start`、`process/read`、`process/terminate`、`fs/readFile`、`fs/writeFile`、`fs/createDirectory`、`fs/remove`，shell 和文件内容工具可用 `AIALRA_EXEC_BACKEND=codex` 优先走 sidecar，并失败回退。 | AIALRA 线上服务已指向 systemd sidecar；bash 和文件工具能优先走 sidecar，Turn Inspector 会记录 executor started/finished/fallback。 | 目录列举还未接 `fs/readDirectory`；远程 environment/HTTP API 还未接。 |
...
| 三方 A/B harness | 每次比较靠手工复制 prompt，结果不可追溯。 | `node aialra/turn-observability/scripts/run-ab-comparison.mjs` 现在默认跑 7 个 prompt：5 个标准任务 + 2 个口语化/情绪化真实任务，分别跑 Codex CLI、debug1 原版 OpenCode、AIALRA fork，并生成 Markdown 报告。 | 目前是行为级 A/B，不是 SWE-bench；原版 OpenCode 没有 public event，所以 turn 终态只能近似判断。 | 看 `aialra/turn-observability/ab-reports/`。 | 报告会自动写每场和整体“谁更好、为什么”。 |
| 最新 A/B 结果 | 之前第 4 个越界写入场景里，AIALRA 已挡住外部写入，但模型随后只读确认外部路径时触发 OpenCode 旧 `external_directory` 审批，脚本判定为等待审批。 | TurnContext 存在时，外部路径读写是否允许由本轮 sandbox 决定：外部写仍被拒绝，外部只读确认不再把 turn 卡成审批。 | Codex CLI 和 AIALRA 在 7 个行为场景里同分；AIALRA 仍比 Codex 少 `fs/readDirectory`、remote environment 和完整 item lifecycle。 | 看 `ab-reports/ab-comparison-20260519153559.md`。 | Codex CLI：7/7；AIALRA：7/7；debug1 原版：4/7。AIALRA 0 卡死、0 等待审批、0 越界写入、7/7 turn 终态。 |
| Codex exec-server 握手诊断 | 只知道“WebSocket 握手失败”，没有证据链。 | 已记录真实原因链：全局 `codex-cli 0.125.0-alpha.3` 的 exec-server 打印 ws 地址，但 `/readyz` 空响应、WebSocket 非 101；源码构建 binary `/srv/aialra/apps/codex-turn-engine/codex-rs/target/debug/codex` 可 `/readyz=200`，可 `initialize`，可 `process/start/read` 和 FS API。 | 还没有接 Codex remote environment、HTTP API 和 `fs/readDirectory`。 | 看 `test/tool/codex-exec-server.test.ts` 和 systemd sidecar 状态。 | live adapter test 通过：process 和 FS API 均通过。 |
...
当前明确未完成：目录列举尚未接 `fs/readDirectory`；Landlock 尚未在 Node/Bun 工具层直接 syscall enforce；Codex remote/multi-environment、HTTP API、实时 stdout/stderr 分片 UI 还未完整产品化；审批 reviewer 语义还未做到 Codex 1:1。
...
| 文件工具 FS API | read/write/edit/apply_patch 只用 Node/Bun 文件系统。 | 在有 TurnContext 且启用 Codex backend 时，文件内容读取、写入、创建目录、删除优先走 Codex exec-server FS API；TurnContext cwd/permission/sandbox/approval 门禁不丢。 | read 工具的目录列举还未接 `fs/readDirectory`；edit/apply_patch 的 diff 计算仍在 Node 层。 | Turn Inspector 会出现 `executor.started/finished`，摘要包含 `fs/readFile` 或 `fs/writeFile`。 | live exec-server FS + sandbox + external-directory 测试 26 pass。 |
...
| A/B prompt 覆盖 | 只跑 5 个强约束任务。 | 默认 7 个任务，新增口语化和情绪化 prompt，并自动给每场与整体写“谁更好、为什么”。 | 还不是 SWE-bench；后续要做代码修复 benchmark。 | 看新生成的 `ab-comparison-*.md`。 | 最新 `ab-comparison-20260519153559.md`：Codex CLI 7/7，AIALRA 7/7，debug1 原版 4/7。 |
