...
| A/B prompt 覆盖 | 只跑 5 个强约束任务。 | 默认 9 个任务，新增口语化、情绪化、网络关闭和弱模型循环风险 prompt，并自动给每场与整体写“谁更好、为什么”。 | 还不是 SWE-bench；后续要做代码修复 benchmark。 | 看新生成的 `ab-comparison-*.md`。 | 最新 `ab-comparison-20260519201455.md`：Codex CLI 9/9，AIALRA 9/9，debug1 原版 5/9。 |

## 0.5 2026-05-19 Sandbox Control Center 与产品化安全开关验收

| 项目 | 原来是什么 | 现在是什么 | 和 Codex 还差什么 | 用户怎么观察 | 测试结果 |
| --- | --- | --- | --- | --- | --- |
| Sandbox Control Center（沙盒控制中心） | 安全规则主要藏在后端配置和 trace 里，用户看不到也改不了。 | Turn Inspector 顶部新增控制区，显示并可切换 `permission_profile`（权限档位）、`approval_policy`（审批策略）、`network access`（网络访问）、`exec backend`（执行后端）和 local environment（本地环境）。 | 当前只支持本地 default environment；remote environment 仍显示为未支持。 | 打开回合检查器，顶部会看到“沙盒控制中心”。 | app typecheck 通过，线上部署后需要浏览器 smoke 复验。 |
...
| 安全审计事件 | 权限变更没有 public event。 | 新增 `sandbox.profile.changed`、`sandbox.network.changed`、`approval.policy.changed`、`executor.backend.changed`、`environment.selected`。 | 还需要把“谁审批了危险升级”的 reviewer 语义补齐。 | Turn Inspector 事件流会显示中文摘要，如“网络访问从 restricted 切换到 enabled”。 | `httpapi-public-event.test.ts` 4 pass。 |
