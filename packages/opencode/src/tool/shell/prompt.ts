...
export function parameterSchema(description: string) {
  return Schema.Struct({
    ...
    yield_time_ms: Schema.optional(PositiveInt).annotate({
      description:
        "Optional foreground wait in milliseconds before returning a still-running process_id. Defaults to 10000ms and is clamped to 250-30000ms.",
    }),
...
Usage notes:
  ...
  - You can specify optional yield_time_ms in milliseconds. It controls how long to wait before returning a running process_id for long commands. It defaults to 10000ms and is clamped to 250-30000ms.
