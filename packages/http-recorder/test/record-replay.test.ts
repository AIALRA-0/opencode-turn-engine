...
describe("http-recorder", () => {
  ...
  test("redacts URL credentials", () => {
    expect(HttpRecorder.redactUrl("https://user@example.test/path?safe=value")).toBe(
      "https://%5BREDACTED%5D@example.test/path?safe=value",
    )
  })
        ...
        interactions: [
          {
            ...
            request: {
              method: "POST",
              url: "https://example.test/path?key=redacted-openai-key",
              headers: {},
              body: JSON.stringify({ nested: "redacted-google-key" }),
            },
            response: {
              status: 200,
              headers: {},
  ...
  test("detects secret-looking values inside metadata", () => {
    expect(
      HttpRecorder.cassetteSecretFindings({
        version: 1,
        metadata: { token: "redacted-openai-key" },
        interactions: [],
      }),
    ...
    await run(
      Effect.gen(function* () {
        const exit = yield* Effect.exit(
          post("https://example.test/echo?api_key=secret-value", { step: 3, token: "redacted-openai-key" }),
        )
        ...
        expect(message).not.toContain("redacted-openai-key")
      }),
    )
  })
})
