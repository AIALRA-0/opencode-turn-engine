...
async function main() {
  ...
  await $`git clone https://x-access-token@github.com/${FORK_REPO}.git ${workDir}`
  ...
  const branches = await $`git ls-remote --heads https://x-access-token@github.com/${FORK_REPO}.git`.text()
  ...
  if (oldBranches.length > 0) {
    ...
    for (const branch of oldBranches) {
      await $`git push https://x-access-token@github.com/${FORK_REPO}.git --delete ${branch}`
      console.log(`✅ Deleted branch ${branch}`)
    }
  }

  console.log(`🚀 Pushing to fork...`)
  await $`git push https://x-access-token@github.com/${FORK_REPO}.git ${branchName}`
